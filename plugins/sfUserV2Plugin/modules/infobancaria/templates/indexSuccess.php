...
<script type="text/javascript">
    $(document).ready(function() {
        <?php if(sfContext::getInstance()->getUser()->getAttribute('idProfile') > 2 && !sfContext::getInstance()->getUser()->hasCredential(''.$module.'_insert_1')): ?>
                $(".eliminarFila").css('display', 'none');
                $("#agregarFila").css('display', 'none');
                $("#enviarDados").css('display', 'none');
                $('#title_module input[type=text]').attr('disabled','true');
        ...
        <?php if(sfContext::getInstance()->getUser()->getAttribute('new_user') == sfContext::getInstance()->getUser()->getAttribute('idUserPanel')): ?>
            <?php if(!sfContext::getInstance()->getUser()->hasCredential(''.$module.'_insert_2')): ?>
                $(".eliminarFila").css('display', 'none');
                $("#agregarFila").css('display', 'none');
                $("#enviarDados").css('display', 'none');
                $('#title_module input[type=text]').attr('disabled','true');
...
<div id="title_module">
    <div id="renglon">
        <?php include_partial('lxuser/menu') ?>
    </div>
    <div id="info_pessoais">
        ...
        <form action="" method="post">            
                ...
                <?php if($contasPessoa): ?>
                    <?php foreach ($contasPessoa as $conta): ?>
                        <tr>
                            ...
                            <td>
                                <select name="id_banco[]" id="id_banco" class="id_banco">
            ...
            <input type="submit" value="Enviar dados" id="enviarDados" />
        </form>
    </div>    
</div>

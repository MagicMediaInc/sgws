...
<div id="title_module">
    ...
    <form id="album_seccion" action="<?php echo url_for('lxsection/saveAsingAlbum?id='.$sf_request->getParameter('id')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<div class="frameForm" align="center">
    ...
    <tbody>
        <tr>
            ...
            <td valign="top">
                <b>Galerias actuales para a seção: <?php echo $namesection['name_section'] ?> </b><br /><br />
                <?php if($albunesActuales): ?>
                    
                    <?php foreach ($albunesActuales as $val): ?>
                        <?php echo $val['album'] ?> 
                        <?php echo link_to(__('Delete'),'lxsection/deleteSectionAlbum?id='.$val['id_seccion_album'], array('method' => 'delete', 'class' => 'delete' , 'confirm' => __('Tem certeza de que quer apagar os dados selecionados?'))) ?>
                        <br />
                    <?php endforeach; ?>
                <?php else: ?>

...
//		}
        ...
        if($request->getParameter('q') == 'pj')
        {
            ...
            $cPj = $c->getNewCriterion(EquipeTarefaPeer::CODIGOFUNCIONARIO, aplication_system::getUser(), Criteria::EQUAL);
            $cPj->addOr($c->getNewCriterion(PropostaPeer::GERENTE, aplication_system::getUser() , Criteria::EQUAL));
            $c->add($cPj);
        ...
        if($this->getRequestParameter('buscador'))
        {
            ...
            $criterio->addOr($c->getNewCriterion(PropostaPeer::CLIENTE, '%'.$this->getRequestParameter('buscador').'%', Criteria::LIKE));
            $criterio->addOr($c->getNewCriterion(PropostaPeer::GERENTE, '%'.$this->getRequestParameter('buscador').'%', Criteria::LIKE));
           ...
           if($status_analisis) // Crea la propuesta
           {
               ...
               if(!$request->getParameter('id_analisis'))
               {
                   echo '
                      ...
                      ';
                   $this->redirect('@default?module=projeto&action=edit&codigo_proposta='.$newProposta->getCodigoProposta());
               }else{
                   echo '
                        <script type="text/javascript"> 
                           parent.location.reload();

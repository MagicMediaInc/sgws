	...
	if (!$this->getRequestParameter('buscador')){
		...
		$c = new Criteria();
                if($this->getRequestParameter('categoria'))
                {
                    ...
                    $categoria = "&categoria=".$this->getRequestParameter('categoria');
                    $this->bus_cat = "&categoria=".$this->getRequestParameter('categoria');
                }else{
                    $categoria = '';
                    $this->bus_cat = '';
                }
        ...
        $this->getUser()->setAttribute('uri_producto','sort='.$this->sort.'&by='.$this->by_page.$buscador.$categoria.'&page='.$this->Productoss->getPage());
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      if($request->getParameter('id')){
        return $this->redirect('@default?module=producto&action=index&'.$this->getUser()->getAttribute('uri_producto'));
      }else{
        return $this->redirect('producto/index');
      }
    }
